document.addEventListener('DOMContentLoaded', () => {
  ...
  const encabezados = [
    ...
    "Sexo",
    "Programa Académico",
    "Institución de Procedencia Estancia",
    "País de Procedencia",
    "Modalidad",
    "Personal Responsable (grado: Lic, Mtro., Doc, etc.)",
    "Categoría del personal responsable",
    "Se hospeda en el TESJo",
    "Periodo en que se Hospeda en el TESJo"
  ];
  ...
  function agregarFila() {
    ...
    encabezados.forEach((enc, idx) => {
      ...
      if (enc === "Numero") {
        // Número automático
        celda.textContent = tbody.rows.length;
      } 
      else if (enc === "Sexo") {
  ...
  function actualizarTotales() {
    ...
    Array.from(tbody.rows).forEach((fila, idx) => {
      const sexo = fila.cells[2].firstChild?.value || "";
      const modalidad = fila.cells[6].firstChild?.value || "";
      ...
      if (sexo) {
        if (pais && pais !== "méxico" && pais !== "mexico") {
          if (sexo === "Hombre") resumenLugar.Internacional.hombres++;
          if (sexo === "Mujer") resumenLugar.Internacional.mujeres++;
        } else {
          if (sexo === "Hombre") resumenLugar.Nacional.hombres++;
          if (sexo === "Mujer") resumenLugar.Nacional.mujeres++;
        }
      }

      // Actualizar número de fila
      fila.cells[0].textContent = idx + 1;
    });
    ...
    Array.from(tbody.rows).forEach(fila => {
      const nombre = fila.cells[7].firstChild?.value.trim();
      const categoria = fila.cells[8].firstChild?.value.trim();
      if (nombre) {
        const key = `${nombre}__${categoria}`;
        if (!responsables[key]) {
          responsables[key] = { nombre, categoria, total: 0 };
        }
        responsables[key].total++;
      }
    });

    Object.values(responsables).forEach(r => {
      const fila = document.createElement('tr');
      fila.innerHTML = `
        <td>${r.nombre}</td>
        <td>${r.categoria}</td>
