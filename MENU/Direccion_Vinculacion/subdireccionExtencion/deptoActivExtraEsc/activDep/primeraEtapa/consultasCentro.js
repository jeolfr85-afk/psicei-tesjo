const talleresBase = [
  ...
  "FUTBOL", "VOLEIBOL", "BASQUETBALL", "FUTBOL BARDAS", "TENIS DE MESA", "BASQUETBOL", "OTROS"
];
...
document.getElementById('btnGuardarBD').addEventListener('click', () => {
  ...
  for (let i = 0; i < filas.length; i += 2) {
    ...
    const programa = trH.children[0].textContent.trim();
    const filaH = [...trH.children].map(td => td.textContent.trim());
    const filaM = [...trM.children].map(td => td.textContent.trim());

    datos.push({ programa, sexo: 'HOMBRE', valores: filaH });
    datos.push({ programa, sexo: 'MUJER', valores: filaM });
  }

  localStorage.setItem('datosTalleres', JSON.stringify(datos));
  alert("✅ Datos guardados en localStorage.");
});

// Exportar a PDF (pantalla completa en horizontal)
...
document.getElementById('btnGuardarExcel').addEventListener('click', () => {
  ...
  filas.forEach(tr => {
    const fila = [...tr.children].map(td => td.textContent.trim());
    ws_data.push(fila);
  });

  const ws = XLSX.utils.aoa_to_sheet(ws_data);
  XLSX.utils.book_append_sheet(wb, ws, "Talleres");
  XLSX.writeFile(wb, "reporte_talleres.xlsx");
});

// Cancelar: limpiar celdas editables
